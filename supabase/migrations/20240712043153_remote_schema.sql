    ...
    ORDER BY
        CASE
            WHEN sort = 'rating' THEN (
                SELECT pr.rating_average
                FROM public.photospot_rating_stats pr
                WHERE pr.id = p.id
            )
            WHEN sort = 'nearby' AND latt IS NOT NULL AND lngg IS NOT NULL THEN ST_Distance(p.location::geography, ST_SetSRID(ST_MakePoint(lngg, latt), 4326))
            WHEN sort = 'new' THEN p.created_at
            ELSE NULL
        END DESC,
        p.created_at DESC
