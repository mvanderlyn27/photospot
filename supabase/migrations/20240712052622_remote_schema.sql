...
BEGIN
    ...
    SELECT p.id, p.created_at, p.neighborhood, p.location_name, p.address,
           ...
           CASE
               WHEN latt IS NOT NULL AND lngg IS NOT NULL THEN ST_Distance(p.location::geography, ST_SetSRID(ST_MakePoint(latt,lngg), 4326))
               ELSE NULL
           END as distance,
    ...
    WHERE
        ...
        AND
        ((latt IS NULL OR lngg IS NULL) OR (maximumDistance IS NULL OR (ST_Distance(p.location::geography, ST_SetSRID(ST_MakePoint(latt,lngg), 4326)) <= maximumDistance)))
    ORDER BY
    COALESCE(
        (
        SELECT
        ...
        SELECT
            ...
            ST_SetSRID (ST_MakePoint (latt, lngg), 4326)
            )
        FROM
            public.photospots p
        WHERE
