drop function if exists "public"."search_photospots"(page_size integer, page_count integer, tags bigint[], minimumrating double precision, maximumdistance double precision, latt double precision, lngg double precision, sort text);
...
CREATE OR REPLACE FUNCTION public.search_photospots(page_size integer DEFAULT 20, page_count integer DEFAULT 1, tags bigint[] DEFAULT NULL::bigint[], minimumrating double precision DEFAULT NULL::double precision, maximumdistance double precision DEFAULT NULL::double precision, latt double precision DEFAULT NULL::double precision, lngg double precision DEFAULT NULL::double precision, sort text DEFAULT NULL::text)
 RETURNS TABLE(id bigint, created_at timestamp with time zone, neighborhood text, location_name text, address text, lat double precision, lng double precision, distance double precision, rating double precision)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
    SELECT p.id, p.created_at, p.neighborhood, p.location_name, p.address,
           ...
           CASE
               WHEN lat IS NOT NULL AND lng IS NOT NULL THEN ST_Distance(p.location::geography, ST_SetSRID(ST_MakePoint(lng, lat), 4326))
               ELSE NULL
           END as distance,
