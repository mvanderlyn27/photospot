...
CREATE OR REPLACE FUNCTION public.get_saved_photospots(user_id uuid, page_size bigint DEFAULT 20, page_count bigint DEFAULT 1)
 RETURNS TABLE(top_photoshot_id bigint, top_photoshot_path text, id bigint, lat double precision, lng double precision, neighborhood text, location_name text, address text)
 LANGUAGE sql
AS $function$
