"use server"
import { uploadPhotobookPictureSchema } from "@/components/photobook/uploadPhotobookPictureDialog";
...
export default async function uploadPhotobookPicture(photobookPictureInfo: z.infer<typeof uploadPhotobookPictureSchema>, photospot_id: number, photobookPictures: FormData): Promise<PhotobookPicture> {
    ...
    const fileUploadPromiseArray = photoData.map((photo) => {
        ...
        let photo_path = photospot_id + '/photobook/' + uploadData[0].id + '/' + photo.name;
    ...
    const { data: photobookPictureWithPhotoPath, error: updatePhotopathError } = await supabase.from('photospots_photobook_pictures').update(photo_path_update).eq('id', uploadData[0].id).select('*').single();
