...
export async function getPhotoshotTags(photoshotId: number): Promise<string[]> {
    const supabase = createClient()


    //Need to get all photoshots for a photospot, and display highest quantity tags 

    const { data, error } = await supabase.from('photoshot_tags').select(`id, tags(name)`).eq('id', photoshotId);
    if (error) {
