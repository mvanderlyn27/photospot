...
export default async function editPhotospot(photospotId: number, photospotInfo: z.infer<typeof editPhotospotSchema>, photospotPictures: FormData): Promise<Photospot> {
    ...
    if (photospotInfo?.photosToRemove && photospotInfo.photosToRemove.length > 0) {
        let photo_path = photospotInfo.photosToRemove.map((name: string) => photospotId + '/' + name);
    ...
    const { data: editData, error: uploadError } = await supabase
        ...
        }).eq('id', photospotId).select('*').single();
    if (uploadError) {
