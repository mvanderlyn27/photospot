...
export async function POST(request: NextResponse) {
    ...
    const photospot_id= formData.get('photospot_id')?.toString();
    ...
    if(photospot_id != null && (photos || review_info_raw)) {
        ...
        if(photos!= null){
            const photo_path = `photospots/${photospot_id}/${user.id}`;
        ...
        const {data: reviewtWithPhotoPath, error: updateError} = await supabase.from('photospot_reviews').update(review_data).eq('created_by', user.id).eq('photospot_id',photospot_id).select('*');    
        if(updateError){
            console.log('error updating', updateError);
            return NextResponse.json(updateError,{status: 500});
        }
        console.log('update complete', reviewtWithPhotoPath);
        return NextResponse.json(reviewtWithPhotoPath[0], {status: 200})
    }
    console.log('Missing required info for update', 'id: ',photospot_id, 'photos: ',photos, 'update info: ',review_info_raw)
