...
export async function POST(request: NextResponse) {
    ...
    if(raw_review_data !== null){
        ...
        if(photos!== null){
            const photo_path = `/photospots/${review_info.id}/${data[0].id}`;
            const { error: storageError }= await supabase.storage.from(bucket).upload(photo_path, photos);
            if(storageError) {
        ...
        const photo_path_url = supabase.storage.from(bucket).getPublicUrl(String(photo_path)).data.publicUrl;
        const photo_path_data = {photo_paths: [photo_path_url]};
        const {data: updatedPhotoPathData, error: updatedPhotoPathError} = await supabase.from('photospot_reviews').insert(photo_path_data).select('*');
