...
export async function POST(request: NextResponse) {
    ...
    const photospot_id = body.photospot_id;
    ...
    const {error: databaseError} = await supabase.from('photospot_reviews').delete().eq('photospot_id',photospot_id).eq('created_by',user.id);    
    if(databaseError){
