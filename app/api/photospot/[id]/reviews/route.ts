...
export async function GET(request: Request, { params }: { params: { id: number } }) {
    ...
    const { data, error } = await supabase.from('photospot_reviews').select('*, ...profiles(username)').eq('photospot_id', params.id);
    if (error) {
    ...
    let userReview: Review | undefined = updateReviews.find((review: any) => review.created_by === userData.user?.id);
