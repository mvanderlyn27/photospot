...
export default function PhotospotCard({ photospot }: { photospot: Photospot }) {
  const {
    ...
    isLoading,
    error,
  } = useSWR("/api/photospot/" + photospot.id + "/topPhotoshot", fetcher);
  return (
    <div>
      <div className="flex">
        <div className="w-1/2 lg:h-96">
          {topPhotospot && (
            <img
              ...
              alt="Image"
              src={topPhotospot.photo_paths[0]}
            />
          )}
          {isLoading && (
            <Skeleton className="w-full h-full rounded-md object-cover bg-black/10" />
          )}
        </div>
        <div className="w-1/2 pl-4 flex flex-col gap-4">
          ...
          <p className="text-gray-600">
            {photospot ? photospot.rating_average : ""}
          </p>
        </div>
      </div>
    </div>
