...
export default function EditPhotoshotForm({
  ...
  const { data: photoshots, mutate: mutatePhotoshots } = useSWR(
    () => "/api/photospot/" + photoshot.photospot_id + "/photoshots",
    fetcher
  );

  const [loading, setLoading] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const initialTags = photoshot?.tags
    ? photoshot.tags.map((tag: Tag) => createOption(tag))
    : null;
  ...
  const onEdit = async (
    ...
    updatePhotoshot(handleEdit(data), {
      optimisticData: (currentPhotoshot: Photoshot) => ({
        ...currentPhotoshot,
        ...tempPhotoshot,
      }),
      populateCache: (currentPhotoshot: Photoshot) => ({
        ...currentPhotoshot,
        ...tempPhotoshot,
      }),
    }).then(() => {
  ...
  const setTagError = (tagError: Error) => {
    ...
    if (tagError) {
      editPhotoshotForm.setError("tags", {
        type: "manual",
  ...
  const getInitialFiles = async () => {
    let fileAr: File[] = [];
    if (photoshot) {
      const promiseAr: Promise<void>[] = photoshot.photo_paths.map(
        async (path: string) => {
